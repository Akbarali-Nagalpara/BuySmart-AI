...
public class ProductServiceImpl implements ProductService {
    ...
    public void saveRawCache(RawCacheDTO dto) {
        ...
        // 2️⃣ Declare the variable OUTSIDE the if-block
        ProductRawDataCache cache;

        // 3️⃣ Check existing cache
        ...
        if (existingOpt.isPresent()) {
            ...
            log.info("Updating existing raw cache entry.");
        } else {
            cache = new ProductRawDataCache();
            cache.setExternalProductId(dto.getProductId());
            log.info("Creating new raw cache entry.");
        }

        // 4️⃣ Set fields
        cache.setRawJson(rawMap); // ✔ this is a Map, stored as JSONB
        ...
        log.info("Raw cache saved for productId {} (expires at {})",
                dto.getProductId(), cache.getExpiryAt());
    }
